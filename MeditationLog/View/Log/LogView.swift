                ...
                if let sideEffects = vm.selectedDayRecord?.sideEffects, (sideEffects.filter({ $0.count != 0 }).count >= 1) {
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 20)
                            .foregroundStyle(Color.ML_Primary.opacity(0.10))
                    )
                } else {
    ...
    private var recordView: some View {
        VStack (spacing: 4) {
            ForEach(hoursOfDay, id: \.self) { hour in
                ...
                Divider().foregroundStyle(Color.ML_DarkBlue.opacity(0.5))
            }
        }
    }
}
